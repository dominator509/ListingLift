...
- **Symptom**: mixed style across files (e.g., TypeScript `camelCase` mixed with `snake_case`).
- **Detect**: language-specific lint (e.g., `tsc --noEmit && eslint . --max-warnings 0`).
...
- **Rollback**: tests are additive; rollback rarely needed. If FLAKY → `test.skip(` or `it.skip(` + log new LEGACY-GAP.

## Category 7 — Dependency Drift
- **Symptom**: unused or outdated dependencies.
- **Detect**: `pnpm outdated` / `depcheck` / `npm outdated`.
...
- **Rollback**: `git checkout HEAD~1 -- package.json pnpm-lock.yaml` (or equivalent).
