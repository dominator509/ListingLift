...
- **Detect**: no `dist/` or `.next/` directory.
- **Close**: `pnpm build`.
- **Verify**: build output in `dist/` or `.next/`.
- **DEC template**: scope `<phase>/build`.

## Category 5 — Test Execution Gaps
- **Symptom**: unit tests authored, never run.
- **Detect**: no `pnpm test` artifact (no vitest/jest output).
- **Close**: `pnpm test -- --reporter=verbose`.
- **Verify**: all tests pass.
- **DEC template**: scope `<phase>/test`.

## Category 6 — Integration Test Gaps
- **Symptom**: tests gated on credentials.
- **Detect**: `.skip` or `it.skip(` annotations + GAP-id comment.
- **Close**: close Category 1 first, then remove `.skip` and run `pnpm test`.
