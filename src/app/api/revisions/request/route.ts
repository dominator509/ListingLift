...
import { createRevisionRequestSchema } from '@/schemas/manual-approval';
...
export async function POST(request: Request) {
  try {
    ...
    assertPermission(session, 'request:revisions');
    ...
    const payload = createRevisionRequestSchema.parse(await parseJson(request, {}));
    const result = { revision: buildRevisionRequestDraft(payload, { organizationId: session.organizationId, actorUserId: session.userId }), note: 'Revision request with idempotency guard.' };
