...
export async function POST(request: Request) {
  ...
  if (!payload.trim()) return jsonFail('empty_gumroad_payload', 'Gumroad webhook payload is required.', 400);

  // P3: Verify HMAC-SHA256 signature
  const signatureHeader = request.headers.get('gumroad-signature') ?? request.headers.get('x-gumroad-webhook-signature') ?? request.headers.get('x-gumroad-signature') ?? '';
  const env = getEnv();
...
export async function GET() {
  return jsonOk({ provider: 'gumroad', status: 'active', note: 'Use POST with Gumroad-Signature header for webhook delivery.' });
}
