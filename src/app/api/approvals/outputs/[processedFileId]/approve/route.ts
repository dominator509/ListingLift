...
export async function POST(request: Request, { params }: { params: Promise<{ processedFileId: string }> }) {
  try {
    ...
    const payload = outputApprovalSchema.parse({ ...(await parseJson(request, {})), processedFileId: (await params).processedFileId, decision: 'APPROVE_OUTPUT' });
    const result = { outputApproval: buildOutputApprovalDecision(payload, { organizationId: session.organizationId, jobId, actorUserId: session.userId }), note: 'Output approval with idempotency guard.' };
