...
export async function POST(request: Request, { params }: { params: Promise<{ jobId: string }> }) {
  try {
    ...
    const payload = manualJobApprovalSchema.parse({ ...(await parseJson(request, {})), jobId: (await params).jobId, decision: 'REJECT_JOB' });
    const result = { rejection: buildManualApprovalDecision(payload, { organizationId: session.organizationId, actorUserId: session.userId }), note: 'Job rejection with idempotency guard.' };
