...
- Focused file-storage nested admin E2E coverage now passes across the storage connections, folder-import, and delivery-export shells.
...
- Expanded file-storage admin coverage to include the previously unverified nested `connections`, `folder-import`, and `delivery-export` shells after focused local Playwright verification.
...
- Removed the final conditional `test.skip()` branch from the rate-limiting E2E suite after the repo-local Playwright and DB scripts were updated to honor `.env.test` and the suite passed against the documented Docker PostgreSQL path.
